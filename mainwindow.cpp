...
void MainWindow::setComparisonData()
{
    ...
    // if a pokemon is selected update the lineseries on the polarchart with the correspondent pokemon stats
    if(combobox1->currentIndex() != -1) {
        Pokemon pokemon1 = _pokedex[combobox1->currentIndex()];
        updateSeries(_chart, pokemon1, 60);
    }
    if(combobox2->currentIndex() != -1) {
        Pokemon pokemon2 = _pokedex[combobox2->currentIndex()];
        updateSeries(_chart, pokemon2, 120);
    }
...
void MainWindow::updateSeries(QPolarChart* chart, Pokemon pokemon, unsigned int brightness)
{
    ...
    QLineSeries *series = generateLineSeries(pokemon, max);
    setSeriesColor(series, pokemon, brightness);
